...
app.post('/api/auth/login', rateLimitMiddleware, async (req, res) => {
  try {
    ...
    if (!result.valid) {
      log('warn', 'Failed login attempt', { requestId: req.requestId });
      return res.status(401).json({ error: 'Invalid credentials', code: 'AUTH_FAILED' });
    }

    // Password OK — check if 2FA is required
    const requires2FA = !!process.env.AUTH_TOKEN_HASH;
    if (requires2FA) {
      // Issue a temporary pre-auth token (short expiry, not fully authenticated)
      const preToken = auth.signSession({ role: result.role, stage: 'pre-2fa', exp: Math.floor(Date.now()/1000) + 300 });
      return res.json({
        success: true,
        requires2FA: true,
        preToken,
    ...
    const token = auth.signSession({ role: result.role, authenticated: true });
    log('info', `Login successful: ${result.role}`, { requestId: req.requestId });
    res.json({ success: true, token, role: result.role });
  } catch (error) {
    log('error', `Auth error: ${error.message}`, { requestId: req.requestId });
    res.status(500).json({ error: 'Authentication error', code: 'AUTH_ERROR' });
  }
});

// Step 2: Verify 2FA token
app.post('/api/auth/verify-2fa', rateLimitMiddleware, async (req, res) => {
  try {
    const { token: tfaToken, preToken } = req.body;
    if (!tfaToken || !preToken) {
      return res.status(400).json({ error: 'Token and preToken required', code: 'MISSING_FIELDS' });
    }

    // Verify pre-auth token
    const pre = auth.verifySession(preToken);
    if (!pre || pre.stage !== 'pre-2fa') {
      return res.status(401).json({ error: 'Invalid or expired pre-auth token', code: 'PRE_AUTH_INVALID' });
    }

    // Verify 2FA token
    const valid = await auth.verifyToken(tfaToken);
    if (!valid) {
      return res.status(401).json({ error: 'Invalid 2FA token', code: '2FA_FAILED' });
    }

    // Issue full session token
    const sessionToken = auth.signSession({ role: pre.role, authenticated: true });
    log('info', `2FA verified: ${pre.role}`, { requestId: req.requestId });
    res.json({ success: true, token: sessionToken, role: pre.role });
  } catch (error) {
    log('error', `2FA error: ${error.message}`, { requestId: req.requestId });
    res.status(500).json({ error: '2FA verification error', code: '2FA_ERROR' });
  }
});
...
app.post('/api/auth/logout', auth.requireAuth, (req, res) => {
  log('info', `Logout: ${req.user.role}`, { requestId: req.requestId });
...
// Clear sports cache
app.post('/api/sports/cache/clear', auth.requireAuth, (req, res) => {
  try {
...
// Validate path is within allowed directory (hardened)
const FORGE_MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB limit
const FORGE_BLOCKED_EXTENSIONS = ['.env', '.key', '.pem', '.cert', '.secret'];
...
app.post('/api/forge/save-file', auth.requireAuth, async (req, res) => {
  try {
...
app.post('/api/forge/read-file', auth.requireAuth, async (req, res) => {
  try {
...
// LIST DIRECTORY
app.post('/api/forge/list-directory', auth.requireAuth, async (req, res) => {
  try {
...
// DELETE FILE
app.post('/api/forge/delete-file', auth.requireAuth, async (req, res) => {
  try {
    const { filePath } = req.body;
...
// CREATE CALL SHEET
app.post('/api/callsheets/create', auth.requireAuth, (req, res) => {
  try {
    const callSheet = {
      id: `cs-${Date.now()}`,
      ...req.body,
...
// REPORT INCIDENT
app.post('/api/safety/incident', auth.requireAuth, (req, res) => {
  try {
    const incident = {
      id: `inc-${Date.now()}`,
      ...req.body,
